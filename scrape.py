...
parser.add_argument(
    "-item", "--item", help="enter the item you want to search for (*)", type=str, nargs="+")
parser.add_argument(
    "-min", "--lower", help="enter the lower bound product price", type=int)
parser.add_argument(
    "-max", "--upper", help="enter the upper bound product price", type=int)
parser.add_argument(
    "-num", "--num", help="enter the number of links you want the program to look through (recommended 100+) (*)",
    type=int)
parser.add_argument(
    "-save", "--out", help="enter the name of the csv/json you want the product information to go to", type=str)
parser.add_argument("-c", dest="cheap",
                    help="add this argument if you want the program to return the cheapest item at the end of scraping",
                    action="store_true")
...
class Scraper():
    ...
    def scrape(self):
        ...
        print(GREEN + "It took %s seconds " % (
                    time.time() - start_time) + f"to process {self.starting_amount} relevant items that were found." + NORM)
        # Soup is done
    ...
    def process_item_links(self):
        ...
        while True:
            ...
            if not productPrice.isalnum():
                ...
                if (self.args.lower and self.args.lower != 0) and (
                        self.args.upper and self.args.upper != 0) and self.args.lower <= productPrice <= self.args.upper:
                    item = Item(self.itemNum, productTitle, "$" + str(productPrice),
                                productRating, numberOfReviews, productAvailability, URL)
                    self.allItems.append(item)
                elif (
                        self.args.lower == None or self.args.lower == 0) and self.args.upper and productPrice <= self.args.upper:
                    item = Item(self.itemNum, productTitle, "$" + str(productPrice),
                                productRating, numberOfReviews, productAvailability, URL)
                    self.allItems.append(item)
                elif (
                        self.args.upper == None or self.args.upper == 0) and self.args.lower and self.args.lower <= productPrice:
                    item = Item(self.itemNum, productTitle, "$" + str(productPrice),
                                productRating, numberOfReviews, productAvailability, URL)
                    self.allItems.append(item)
                elif (self.args.upper == None or self.args.upper == 0) and (
                        self.args.lower == None or self.args.lower == 0):
                    item = Item(self.itemNum, productTitle, "$" + str(productPrice),
                                productRating, numberOfReviews, productAvailability, URL)
                    self.allItems.append(item)
                self.itemNum += 1
